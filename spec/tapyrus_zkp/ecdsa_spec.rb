...
RSpec.describe TapyrusZkp::ECDSA do
  ...
  describe 'Signature#from_der' do
    context 'Invalid signature' do
      it 'should raise InvalidSignature' do
        sig = hex!('00' * TapyrusZkp::ECDSA::Signature::SIZE_SERIALIZED)
        expect { TapyrusZkp::ECDSA::Signature.from_der(ctx, sig) }.to raise_error(TapyrusZkp::InvalidSignature)
        sig = hex!('00' * (TapyrusZkp::ECDSA::Signature::SIZE_SERIALIZED + 1))
        expect { TapyrusZkp::ECDSA::Signature.from_der(ctx, sig) }.to raise_error(TapyrusZkp::InvalidSignature)
      end
    end
  end
  ...
  describe 'signature serialization round trip' do
    it 'should success' do
      100.times do
        msg = SecureRandom.bytes(TapyrusZkp::PrivateKey::BYTE_SIZE)
        private_key = TapyrusZkp::PrivateKey.generate(ctx)
        sig1 = private_key.sign(ctx, msg)

        # DER
        der = sig1.to_der(ctx)
        sig2 = TapyrusZkp::ECDSA::Signature.from_der(ctx, der)
        expect(sig1).to eq(sig2)

        # Compact
        compact = sig1.to_compact(ctx)
        sig2 = TapyrusZkp::ECDSA::Signature.from_compact(ctx, compact)
        expect(sig1).to eq(sig2)

        expect { TapyrusZkp::ECDSA::Signature.from_der(ctx, compact) }.to raise_error(TapyrusZkp::InvalidSignature)
        expect { TapyrusZkp::ECDSA::Signature.from_compact(ctx, der) }.to raise_error(TapyrusZkp::InvalidSignature)
      end
    end
  end
  ...
  describe 'LOW-S signature' do
    it do
      ...
      signature = TapyrusZkp::ECDSA::Signature.from_der(ctx, sig)
      pubkey = TapyrusZkp::PublicKey.from_hex(ctx, '031ee99d2b786ab3b0991325f2de8489246a6a3fdb700f6d0511b1d80cf5f4cd43')
  ...
  describe 'Recoverable signature' do
    it do
      ...
      private_key = TapyrusZkp::PrivateKey.from_hex(ctx, one)
      msg = hex!(one)
      ...
      sig2 = TapyrusZkp::ECDSA::RecoverableSignature.from_compact(ctx, compact, 1)
      expect(sig1).to eq(sig2)
